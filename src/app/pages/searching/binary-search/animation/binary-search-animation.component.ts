...
@Component({
  selector: 'binary-search-animation',
  templateUrl: './binary-search-animation.component.html',
  styleUrl: './binary-search-animation.component.scss',
})
export class BinarySearchAnimationComponent {
 ...
 private async search(numbersArr: number[], searchedNumber: number) {
  ...
  while (left <= right) {
    ...
    if (numbersArr[mid] === searchedNumber) {
      ((Object.values(this.sqArr[mid])[0] as SquareModel).index.material as THREE.MeshBasicMaterial).color.set(0xeb4034);
      this.renderer.render(this.scene, this.camera);

      return mid; // Element found, return its index
    } else if (numbersArr[mid] < searchedNumber) {
